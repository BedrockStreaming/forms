...
import { CORRECT_DICTIONARY, CORRECT_SCHEMA, typesAllowed } from '../../__tests__/fixtures';
...
describe('handleFormBuilderError', () => {
  it('should not throw a FormBuilderError error if schema, types allowed and dictionary are correct', () => {
    return expect(() => handleFormBuilderError(typesAllowed, CORRECT_SCHEMA, CORRECT_DICTIONARY)).not.toThrowError(
      FormBuilderError,
    );
  });
  ...
  it('should throw a FormBuilderError error when a field has a submit type', () => {
    ...
    return expect(() =>
      handleFormBuilderError([...typesAllowed, 'submit'], badSchema as any, CORRECT_DICTIONARY),
  ...
  it('should throw a FormBuilderError error when a field is not whitelisted', () => {
    ...
    return expect(() => handleFormBuilderError(typesAllowed, badSchema as any, CORRECT_DICTIONARY)).toThrowError(
      FormBuilderError,
    );
  });
  ...
  it('should throw a FormBuilderError error when steps are empty', () => {
    ...
    return expect(() => handleFormBuilderError(typesAllowed, badSchema, CORRECT_DICTIONARY)).toThrowError(
      FormBuilderError,
    );
  });
  ...
  it('should throw a FormBuilderError error when a submit component is missing in dictionary', () => {
    ...
    return expect(() => handleFormBuilderError(typesAllowed, CORRECT_SCHEMA, badDictionary as any)).toThrowError(
      FormBuilderError,
    );
  });
});
